...
TEMPERATURE_DIR = BASE_DIR / "data" / "processed" / "noaa-temperature"
PRECIPITATION_DIR = BASE_DIR / "data" / "processed" / "noaa-precipitation"


@st.cache_data
def load_temperature_data(station_id: str):
    file_path = TEMPERATURE_DIR / f"{station_id}_daily_temperature.csv"
    return _load_noaa_csv(file_path)


@st.cache_data
def load_precipitation_data(station_id: str):
    file_path = PRECIPITATION_DIR / f"{station_id}_daily_precipitation.csv"
...
AIR_QUALITY_DIR = BASE_DIR / "data" / "processed" / "aqs"
LOCATION_SUMMARIES_FILE = BASE_DIR / "data" / "processed" / "location-summaries.json"
...
def load_air_quality_data(
    ...
    if pollutant not in AQS_POLLUTANTS:
        raise ValueError("pollutant must be either 'pm25' or 'ozone'")

    file_path = AIR_QUALITY_DIR / f"aqs-{pollutant}-{aqs_site_id}.csv"
