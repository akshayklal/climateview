...
def station_file_id(station_code: str) -> str:
    """
    Converts:
      USW00023234 -> GHCND_USW00023234
      GHCND:USW00023234 -> GHCND_USW00023234
    """
...
def output_station_id(station_code: str) -> str:
    """
    Converts:
      GHCND:USW00023234 -> USW00023234
      USW00023234 -> USW00023234
    """
    station_code = station_code.strip()
    if station_code.startswith("GHCND:"):
...
def records_to_dataframe(records: List[Dict], value_column: str) -> pd.DataFrame:
    if not records:
...
def build_processed_temperature_data(
    station_code: str,
    start_year: int,
    end_year: int,
) -> pd.DataFrame:
    ...
    for year in range(start_year, end_year + 1):
        tmax_path = raw_file_path("TMAX", file_station_id, year)
        tmin_path = raw_file_path("TMIN", file_station_id, year)

        tmax_records = load_raw_records(tmax_path)
        tmin_records = load_raw_records(tmin_path)

        tmax_df = records_to_dataframe(tmax_records, "tmax_f")
        tmin_df = records_to_dataframe(tmin_records, "tmin_f")

        daily = pd.merge(tmax_df, tmin_df, on="date", how="outer")
        daily["station_id"] = clean_station_id

        all_rows.append(daily)

    if not all_rows:
        return pd.DataFrame(columns=["station_id", "date", "tmax_f", "tmin_f"])

    result = pd.concat(all_rows, ignore_index=True)

    result = result[["station_id", "date", "tmax_f", "tmin_f"]]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build processed daily temperature CSV from raw NOAA TMAX/TMIN JSON files."
    )
